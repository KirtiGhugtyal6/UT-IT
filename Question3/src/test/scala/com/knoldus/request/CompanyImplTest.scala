...
class CompanyImplIntegrationTest extends AnyFlatSpec {
  ...
  "Company" should "not get created as email id is invalid" in {
    val knoldusCompany: Company = Company("Knoldus", "kirti.com", "Noida")
    val result =  companyImpl.createCompany(knoldusCompany)
    assert(result=="")
  }

  "Company" should "not get created as email id is invalid and company already exists" in {
